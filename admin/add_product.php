...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name) || $price <= 0) {
        ...
        if (empty($error)) {
            $sql = "INSERT INTO products (name, description, price, stock, category, photo) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssdiss", $name, $description, $price, $stock, $category, $photo);
...
<div class="admin-form">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-row">
            <div class="form-group">
                <label>Цена *</label>
                <input type="number" name="price" step="0.01" required value="<?= $_POST['price'] ?? '' ?>">
            </div>
            
            <div class="form-group">
                <label>Количество на складе</label>
                <input type="number" name="stock" value="<?= $_POST['stock'] ?? 0 ?>">
            </div>
        </div>
        
        <div class="form-group">
            <label>Категория</label>
            <input type="text" name="category" list="categories" value="<?= htmlspecialchars($_POST['category'] ?? '') ?>">
        ...
        <div class="form-section">
            <h3>📸 Фото товара</h3>
            <p class="text-muted" style="margin-bottom: 16px;">Выберите один из способов добавления фото:</p>
            
            <!-- Способ 1: Загрузка файла -->
            <div class="form-group">
                <label>📁 Способ 1: Загрузить файл с компьютера</label>
                <input type="file" name="photo" id="file-input" accept="image/*">
                <small class="text-muted">Поддерживаются: JPG, JPEG, PNG, GIF, WEBP</small>
            </div>
            ...
            <div class="form-group">
                <label>🔗 Способ 2: Вставить ссылку на фото (URL)</label>
                ...
                <small class="text-muted">Можно вставлять ссылки с Яндекс.Картинок, Google и любых других сайтов</small>
            </div>
            
            <!-- Блок предпросмотра -->
            <div class="form-group" id="preview-block" style="display: none;">
                <label>👁️ Предпросмотр:</label>
...
urlInput.addEventListener('input', function() {
    ...
    if (url && (url.startsWith('http://') || url.startsWith('https://'))) {
        showPreview(url);
        // Очищаем файловый инпут, чтобы не было конфликта
...
fileInput.addEventListener('change', function() {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            // Очищаем URL инпут, чтобы не было конфликта
            urlInput.value = '';
        }
